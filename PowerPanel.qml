...
Panel {
  ...
  readonly property string vivobookApply: root.pluginDir + "/bin/apply"

  function setProfile(profile) {
    if (!profile || actionProc.running) return
    var source = root.discharging ? "battery" : "ac"
    actionProc.command = [
      "bash", "-c",
      'apply="$1"; source="$2"; profile="$3"; if [ -x "$apply" ]; then exec "$apply" --remember --source "$source" "$profile"; else exec omarchy-powerprofiles-set "$source" "$profile"; fi',
      "vivobook-s14-copilot-apply",
      root.vivobookApply,
      source,
      String(profile)
    ]
  ...
  function openCustomize() {
    ...
    var payload = JSON.stringify({ profile: profile })
    root.close()
    var svc = root.bar && root.bar.shell && typeof root.bar.shell.serviceFor === "function"
      ? root.bar.shell.serviceFor("vivobook.s14-copilot")
      : null
    if (svc && typeof svc.openSettings === "function") {
      svc.openSettings(payload)
      return
    }
    Quickshell.execDetached(["omarchy-shell", "vivobook.s14-copilot", "open", payload])
  }
      ...
      Column {
        ...
        Column {
          ...
          Text {
            ...
            textFormat: Text.PlainText
            text: "Extras follow the Vivobook S14 Copilot+ profile. Customize refresh, charge hold, keyboard light, and more."
            color: Qt.darker(root.bar.foreground, 1.4)
            font.family: root.bar.fontFamily
            font.pixelSize: Style.font.caption
          }
